...
Options parse(const int argc, char** const argv) noexcept
{
    Options options{};
    ...
    app.add_option("-f,--factor", options.factor, "Factor for upscaling.")
        ->capture_default_str();
    app.add_option("-t,--threads", options.threads, "Threads for video or multi-input image processing.")
        ->capture_default_str();
    ...
    try { app.parse(argc, app.ensure_utf8(argv)); if (!options.list && options.inputs.empty()) throw CLI::CallForHelp(); }
