...
void Anime4KCPP::NCNN::ACNet::setArguments(const Parameters& parameters)
{
...
void Anime4KCPP::NCNN::ACNet::processRGBVideoB()
{
    if (!param.fastMode)
    {
        int scaleTimes = Utils::fastCeilLog2(param.zoomFactor);
        ...
        videoIO->init(
            [this, scaleTimes]()
            {
                ...
                tmpFrame.convertTo(tmpFrame, CV_32FC1, 1.0 / 255.0);
                if (vkdev == nullptr)
                {
                    processCPU(tmpFrame, dstFrame, scaleTimes, &dataHolder);
                }
                else
                {
                    processVK(tmpFrame, dstFrame, scaleTimes, &dataHolder);
                }
                dstFrame.convertTo(dstFrame, CV_8UC1, 255.0);
    ...
    else
    {
        videoIO->init(
            [this]()
            {
                ...
                orgFrame.convertTo(orgFrame, CV_32FC1, 1.0 / 255.0);
                if (vkdev == nullptr)
                {
                    processCPU(orgFrame, dstFrame, 1, &dataHolder);
                }
                else
                {
                    processVK(orgFrame, dstFrame, 1, &dataHolder);
                }
                dstFrame.convertTo(dstFrame, CV_8UC1, 255.0);
