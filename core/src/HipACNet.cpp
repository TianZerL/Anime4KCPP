...
namespace Anime4KCPP::Hip::detail
{
    static void runKernel(const cv::Mat& orgImg, cv::Mat& dstImg, int scaleTimes, int index)
    {
        ...
        void (*cuRunKernelACNet)(const void*, void*, ACHipDataType, ACHipParamACNet*);
        ...
        for (int i = 0; i < scaleTimes; i++)
        {
            ...
            ACHipParamACNet cuParam{ tmpImg.cols, tmpImg.rows, tmpImg.step };
...
Anime4KCPP::Hip::ACNet::ACNet(const Parameters& parameters) :
    AC(parameters), ACNetTypeIndex(GET_ACNET_TYPE_INDEX(param.HDN, param.HDNLevel)) {}

void Anime4KCPP::Hip::ACNet::setParameters(const Parameters& parameters)
{
    AC::setParameters(parameters);
    ACNetTypeIndex = GET_ACNET_TYPE_INDEX(param.HDN, param.HDNLevel);
}

std::string Anime4KCPP::Hip::ACNet::getInfo() const
{
...
std::string Anime4KCPP::Hip::ACNet::getFiltersInfo() const
{
    std::ostringstream oss;
...
void Anime4KCPP::Hip::ACNet::processYUVImage()
{
    if (!param.fastMode)
    {
...
void Anime4KCPP::Hip::ACNet::processRGBImage()
{
    if (!param.fastMode)
    {
        int scaleTimes = Utils::fastCeilLog2(param.zoomFactor);
...
void Anime4KCPP::Hip::ACNet::processGrayscale()
{
    if (!param.fastMode)
    {
        int scaleTimes = Utils::fastCeilLog2(param.zoomFactor);
...
Anime4KCPP::Processor::Type Anime4KCPP::Hip::ACNet::getProcessorType() const noexcept
{
    return Processor::Type::Hip_ACNet;
}

std::string Anime4KCPP::Hip::ACNet::getProcessorInfo() const
{
    std::ostringstream oss;
