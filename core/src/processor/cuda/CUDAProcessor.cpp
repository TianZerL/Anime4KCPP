...
namespace ac::core::cuda
{
    ...
    struct DeviceImageBuffer
    {
        ...
        DeviceImageBuffer(const Image& hostImage, cudaError_t& err) noexcept : err(err)
        {
            std::size_t stride{};
            auto lineSize = hostImage.width() * hostImage.pixelSize();
            err = cudaMallocPitch(&ptr, &stride, lineSize, hostImage.height());
        ...
        DeviceImageBuffer(int w, int h, int c, int elementSize, cudaError_t& err) noexcept : err(err)
        {
            std::size_t stride{};
            auto lineSize = w * c * elementSize;
            err = cudaMallocPitch(&ptr, &stride, lineSize, h);
            if (err != cudaSuccess) return;
...
void ac::core::cuda::CUDAProcessor<ac::core::model::ACNet>::process(const Image& src, Image& dst)
{
    ...
    DeviceImageBuffer tmp2{ src.width(), src.height(), 8, 2, err }; if (err != cudaSuccess) return;
    DeviceImageBuffer out{ dst, err }; if (err != cudaSuccess) return;
...
void ac::core::cuda::CUDAProcessor<ac::core::model::ARNet>::process(const Image& src, Image& dst)
{
    ...
    DeviceImageBuffer tmp1{ src.width(), src.height(), 8, 2, err }; if (err != cudaSuccess) return;
    DeviceImageBuffer tmp2{ src.width(), src.height(), 8, 2, err }; if (err != cudaSuccess) return;
    DeviceImageBuffer feat{ src.width(), src.height(), 8, 2, err }; if (err != cudaSuccess) return;
    DeviceImageBuffer out{ dst, err }; if (err != cudaSuccess) return;
