...
namespace ac::core::cpu
{
    ...
    void conv3x3_8to4_identity_pixelshuffle_4to1_add_wasm_simd128(const Image& src, Image& dst, const float* kernels, const float* biases, const Image& id)
    {
        ...
        {
        case Image::UInt8:
            conv3x3_identity_pixelshuffle_wasm_simd128_float<std::uint8_t, 8, 2>(src, dst, kernels, biases, ResidualArg{ id, 1.0f });
            break;
        case Image::UInt16:
            conv3x3_identity_pixelshuffle_wasm_simd128_float<std::uint16_t, 8, 2>(src, dst, kernels, biases, ResidualArg{ id, 1.0f });
            break;
        case Image::Float32:
            conv3x3_identity_pixelshuffle_wasm_simd128_float<float, 8, 2>(src, dst, kernels, biases, ResidualArg{ id, 1.0f });
            break;
        }
    }
