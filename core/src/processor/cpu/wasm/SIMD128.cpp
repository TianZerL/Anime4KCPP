    ...
    inline void conv3x3_wasm_simd128_float(const Image& src, Image& dst, const float* const kernels, const float* const biases, ActiveFunc&& activeFunc, ResidualArgs&& ...residualArg)
    {
        ...
        util::parallelFor(0, src.height(), [&](const int i) {
            ...
            for (int j = 0; j < src.width(); j++)
            {
                ...
                {
                    v128_t r0 = wasm_v128_load(dptr[0] + idx * vstep);
                    v128_t r1 = wasm_v128_load(dptr[1] + idx * vstep);
                    v128_t r2 = wasm_v128_load(dptr[2] + idx * vstep);
                    v128_t r3 = wasm_v128_load(dptr[3] + idx * vstep);
                    v128_t r4 = wasm_v128_load(dptr[4] + idx * vstep);
                    v128_t r5 = wasm_v128_load(dptr[5] + idx * vstep);
                    v128_t r6 = wasm_v128_load(dptr[6] + idx * vstep);
                    v128_t r7 = wasm_v128_load(dptr[7] + idx * vstep);
                    v128_t r8 = wasm_v128_load(dptr[8] + idx * vstep);
                    ...
                    for (int n = 0; n < cout; n++)
                    {
                        ...
                        sum[n] += wasm_simd128_f32x4_hsum(wasm_f32x4_add(s0, wasm_f32x4_add(s1, s2)));
                    }
                }
                if constexpr (remain)
                {
                    v128_t r0 = wasm_f32x4_make((dptr[0] + count * vstep)[0], remain > 1 ? (dptr[0] + count * vstep)[1] : 0.0f, remain > 2 ? (dptr[0] + count * vstep)[2] : 0.0f, 0.0f);
                    v128_t r1 = wasm_f32x4_make((dptr[1] + count * vstep)[0], remain > 1 ? (dptr[1] + count * vstep)[1] : 0.0f, remain > 2 ? (dptr[1] + count * vstep)[2] : 0.0f, 0.0f);
                    v128_t r2 = wasm_f32x4_make((dptr[2] + count * vstep)[0], remain > 1 ? (dptr[2] + count * vstep)[1] : 0.0f, remain > 2 ? (dptr[2] + count * vstep)[2] : 0.0f, 0.0f);
                    v128_t r3 = wasm_f32x4_make((dptr[3] + count * vstep)[0], remain > 1 ? (dptr[3] + count * vstep)[1] : 0.0f, remain > 2 ? (dptr[3] + count * vstep)[2] : 0.0f, 0.0f);
                    v128_t r4 = wasm_f32x4_make((dptr[4] + count * vstep)[0], remain > 1 ? (dptr[4] + count * vstep)[1] : 0.0f, remain > 2 ? (dptr[4] + count * vstep)[2] : 0.0f, 0.0f);
                    v128_t r5 = wasm_f32x4_make((dptr[5] + count * vstep)[0], remain > 1 ? (dptr[5] + count * vstep)[1] : 0.0f, remain > 2 ? (dptr[5] + count * vstep)[2] : 0.0f, 0.0f);
                    v128_t r6 = wasm_f32x4_make((dptr[6] + count * vstep)[0], remain > 1 ? (dptr[6] + count * vstep)[1] : 0.0f, remain > 2 ? (dptr[6] + count * vstep)[2] : 0.0f, 0.0f);
                    v128_t r7 = wasm_f32x4_make((dptr[7] + count * vstep)[0], remain > 1 ? (dptr[7] + count * vstep)[1] : 0.0f, remain > 2 ? (dptr[7] + count * vstep)[2] : 0.0f, 0.0f);
                    v128_t r8 = wasm_f32x4_make((dptr[8] + count * vstep)[0], remain > 1 ? (dptr[8] + count * vstep)[1] : 0.0f, remain > 2 ? (dptr[8] + count * vstep)[2] : 0.0f, 0.0f);
