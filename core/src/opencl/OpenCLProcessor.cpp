...
#define ContextList (ac::core::opencl::getContextList())
...
namespace ac::core::opencl
{
    ...
    // It's strange to write this way, but it's for lazy loading OpenCL devices, just like we did in singleton mode
    // If we just make `ContextList` as a static variable, and we are using this library from a DLL, it will cause a crash in most Opencl implementation.
    ...
    inline static cl_int init(const std::size_t idx) noexcept {
        if (idx >= ContextList.size()) return CL_DEVICE_NOT_AVAILABLE;
        if (ContextList[idx].ctx() && ContextList[idx].cmdq() && ContextList[idx].program()) return CL_SUCCESS;

        cl_int err = CL_SUCCESS;
        ContextList[idx].ctx = cl::Context{ ContextList[idx].device, nullptr, nullptr, nullptr, &err }; if (err != CL_SUCCESS) return err;
        ContextList[idx].cmdq = cl::CommandQueue{ ContextList[idx].ctx, ContextList[idx].device, 0, &err }; if (err != CL_SUCCESS) return err;
        ContextList[idx].program = cl::Program{ ContextList[idx].ctx, KernelString /*From Kernel.hpp*/, false, &err }; if (err != CL_SUCCESS) return err;
        return ContextList[idx].program.build(ContextList[idx].device);
    }
    ...
    {
    public:
        OpenCLProcessorBase(const int idx) noexcept : Processor(idx), err(CL_SUCCESS) {};
...
{
public:
    OpenCLProcessor(int idx, const model::ACNet& model) noexcept;
...
void ac::core::opencl::OpenCLProcessor<ac::core::model::ACNet>::process(const Image& src, Image& dst) noexcept
{
    ...
    cl::size_type srcRangeW = align(srcW, 32), srcRangeH = align(srcH, 32);
    cl::size_type dstRangeW = align(dstW, 32), dstRangeH = align(dstH, 32);

    cl::Kernel conv3x3_1to8{ ContextList[idx].program, "conv3x3_1to8", &err }; if (err != CL_SUCCESS) return;
    cl::Kernel conv3x3_8to8{ ContextList[idx].program, "conv3x3_8to8", &err }; if (err != CL_SUCCESS) return;
    cl::Kernel deconv2x2_8to1{ ContextList[idx].program, "deconv2x2_8to1", &err }; if (err != CL_SUCCESS) return;

    cl::Image2D in{ ContextList[idx].ctx, CL_MEM_READ_ONLY, {CL_R, channelType(src.type())}, srcW, srcH, 0, nullptr, &err }; if (err != CL_SUCCESS) return;
    cl::Image2DArray tmp1{ ContextList[idx].ctx, CL_MEM_READ_WRITE, {CL_RGBA, CL_HALF_FLOAT}, 2, srcW, srcH, 0, 0, nullptr, &err }; if (err != CL_SUCCESS) return;
    cl::Image2DArray tmp2{ ContextList[idx].ctx, CL_MEM_READ_WRITE, {CL_RGBA, CL_HALF_FLOAT}, 2, srcW, srcH, 0, 0, nullptr, &err }; if (err != CL_SUCCESS) return;
    cl::Image2D out{ ContextList[idx].ctx, CL_MEM_WRITE_ONLY, {CL_R, channelType(dst.type())}, dstW, dstH, 0, nullptr, &err }; if (err != CL_SUCCESS) return;

    err = ContextList[idx].cmdq.enqueueWriteImage(in, CL_FALSE, { 0,0,0 }, { srcW,srcH,1 }, src.stride(), 0, src.ptr()); if (err != CL_SUCCESS) return;
    ...
    err = ContextList[idx].cmdq.enqueueNDRangeKernel(conv3x3_1to8, cl::NullRange, cl::NDRange(srcRangeW, srcRangeH)); if (err != CL_SUCCESS) return;
    for (int i = 0; i < 4; i++)
    {
        ...
        err = ContextList[idx].cmdq.enqueueNDRangeKernel(conv3x3_8to8, cl::NullRange, cl::NDRange(srcRangeW, srcRangeH)); if (err != CL_SUCCESS) return;
    ...
    err = ContextList[idx].cmdq.enqueueNDRangeKernel(deconv2x2_8to1, cl::NullRange, cl::NDRange(dstRangeW, dstRangeH)); if (err != CL_SUCCESS) return;
    err = ContextList[idx].cmdq.enqueueReadImage(out, CL_TRUE, { 0,0,0 }, { dstW,dstH,1 }, dst.stride(), 0, dst.ptr());
}
