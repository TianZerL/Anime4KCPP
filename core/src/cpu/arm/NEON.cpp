...
namespace ac::core::cpu
{
    ...
    void conv3x3_neon_float(const Image& src, Image& dst, const float* kernels, const float* biases)
    {
    ...
    void conv3x3_neon_cin1(const Image& src, Image& dst, const float* kernels, const float* biases)
    {
        int w = src.width(), h = src.height();
        int wstep = src.stride() / src.elementSize();
        int cstep = src.channelSize() / src.elementSize();
    ...
    template <typename OUT, int cin, int cout>
    void deconv2x2_neon_float(const Image& src, Image& dst, const float* kernels)
    {
